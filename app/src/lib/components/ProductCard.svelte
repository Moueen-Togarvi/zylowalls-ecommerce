	...
	<div class="relative flex flex-1 flex-col justify-between p-3.5 text-left sm:p-4">
		<div class="space-y-1">
			...
			<div class="flex items-center justify-between gap-3 pt-1 pb-1">
				<!-- Star Bar -->
				<div
					class="flex h-[18px] flex-1 items-center justify-center gap-0.5 rounded-md border border-[#14352d]/6 bg-[#fbf9f2] px-2 py-0.5 text-center select-none sm:h-[22px] sm:gap-1"
				>
					{#each [1, 2, 3, 4, 5] as starIndex}
						{#if starIndex <= Math.floor(ratingValue)}
							<!-- Full Star -->
							<svg
								class="h-2.5 w-2.5 fill-current text-[#e4b43d] sm:h-3 sm:w-3"
								viewBox="0 0 20 20"
							>
								<path
						...
						{:else if starIndex - 0.5 <= ratingValue}
							<!-- Half Star -->
							<svg
								class="h-2.5 w-2.5 fill-current text-[#e4b43d] opacity-70 sm:h-3 sm:w-3"
								viewBox="0 0 20 20"
							>
								<path
									d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"
						...
						{:else}
							<!-- Empty Star -->
							<svg class="h-2.5 w-2.5 fill-current text-gray-300 sm:h-3 sm:w-3" viewBox="0 0 20 20">
								<path
									d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"
								/>
							</svg>
					...
					{/each}
					<span
						class="ml-0.5 text-[0.55rem] leading-none font-black text-[#14352d]/60 sm:text-[0.62rem]"
